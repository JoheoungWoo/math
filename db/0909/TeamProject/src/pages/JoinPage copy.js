...
// initialForm 객체는 회원가입 입력폼의 초기값을 담고 있다.
// 모든 input 필드의 value와 연결되어, React의 controlled component(제어 컴포넌트) 패턴으로 동작한다.
// agree는 약관 동의 여부를 체크박스로 관리하기 위해 boolean 값으로 설정한다.
...
const JoinPage = () => {
  ...
  // 전체 사용자 목록인 'allUsers'에 저장한다.
  // 방금 가입한 'joinTemp'를 따로 저장해놓는다.
  // 회원가입 완료 후에는 폼의 데이터를 초기화한다.
  // navigate를 사용하여 로그인 페이지로 이동하여
  // 가입할 때 입력했던 데이터를 state 객체로 가입자 정보를 받고 전달한다.
